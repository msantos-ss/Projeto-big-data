...
def plot_pizzas(df_):
    ...
    if len(pizzas) > 0:
        ...
        with col1:
            ...
            ax.set_title("🍕 Sabores de Pizza Mais Vendidos (Top 5)")
...
def plot_bebidas(df_):
    ...
    with col1:
        fig, ax = plt.subplots(figsize=(6, 4))
        sns.barplot(x=bebidas.values, y=bebidas.index, palette="Blues_r", ax=ax)
        ax.set_title("🍹 Bebidas Mais Escolhidas (Top 5)")
        ax.set_xlabel("Quantidade de Vendas")
        ax.set_ylabel("Bebidas")
        st.pyplot(fig)
    with col2:
        bebidas_pct = (bebidas / bebidas.sum()) * 100
        fig, ax = plt.subplots(figsize=(5, 5))
        ax.pie(bebidas_pct, labels=bebidas_pct.index, autopct='%1.1f%%', startangle=90, colors=sns.color_palette("cool"))
...
def plot_sobremesas(df_):
    ...
    with col1:
        fig, ax = plt.subplots(figsize=(6, 4))
        sns.barplot(x=doces.values, y=doces.index, palette="Purples_r", ax=ax)
        ax.set_title("🍰 Sobremesas Mais Escolhidas (Top 5)")
        ax.set_xlabel("Quantidade de Vendas")
        ax.set_ylabel("Sobremesas")
        st.pyplot(fig)
    with col2:
        doces_pct = (doces / doces.sum()) * 100
        fig, ax = plt.subplots(figsize=(5, 5))
        ax.pie(doces_pct, labels=doces_pct.index, autopct='%1.1f%%', startangle=90, colors=sns.color_palette("pastel"))
...
elif grafico == "🍹 Bebidas Mais Escolhidas":
    plot_bebidas(df_filtrado)
elif grafico == "🍰 Sobremesas Mais Escolhidas":
    plot_sobremesas(df_filtrado)
